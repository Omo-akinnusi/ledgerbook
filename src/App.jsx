...
const PAYSTACK_PLANS = {
  monthly:    { code: import.meta.env.VITE_PAYSTACK_PLAN_MONTHLY    || "PLN_riztufvgcixap7k", label:"Monthly",  months:1,  usd:5,  discount:null },
  biannually: { code: import.meta.env.VITE_PAYSTACK_PLAN_BIANNUALLY || "PLN_lr0mhzc8n3wa28h", label:"6 Months", months:6,  usd:27, discount:"Save 10%" },
  annually:   { code: import.meta.env.VITE_PAYSTACK_PLAN_ANNUALLY   || "PLN_cizrk6zouo32rjs", label:"Annual",   months:12, usd:50, discount:"Save 17%" },
};
          ...
          {screen==="pricing" && <>
            ...
            <div style={{ margin:"16px 0 4px" }}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                {[
                  { id:"card",     icon:"wallet", label:"Card",          sub:"Auto-renews" },
  ...
  return (
    <div style={{minHeight:"100vh",position:"relative",overflow:"hidden",
      ...
      <div style={{position:"relative",zIndex:2,textAlign:"center",
        ...
        <div style={{color:"#fff",fontWeight:900,fontSize:22,letterSpacing:"-.5px",textAlign:"center"}}>
          Cash Counter
        </div>
      ...
      <div className="au-card" style={{position:"relative",zIndex:2,margin:"0 auto",width:"100%",maxWidth:420,
        ...
        <div style={{background:"rgba(255,255,255,.97)",borderRadius:28,
          ...
          <div style={{padding:"20px 22px 26px"}}>
            ...
            {/* Form fields */}
            {mode==="register" && <>
              <AuthField label="Full Name"     placeholder="e.g. Oluwasegun Akinnusi" value={form.name}         onChange={v=>f("name",v)}         icon="person"/>
              <AuthField label="Business Name" placeholder="e.g. Ade Electronics"     value={form.businessName} onChange={v=>f("businessName",v)} icon=""/>
            </>}
            <AuthField label="Email Address"    placeholder="you@example.com"         type="email"    value={form.email}    onChange={v=>f("email",v)}    icon="mail"/>

            {mode !== "forgot" && <>
              <AuthField label="Password" placeholder="Min. 8 chars, uppercase, number" type="password" value={form.password} onChange={v=>f("password",v)} icon="lock"/>
              ...
              {mode === "register" && <AuthField label="Confirm Password" placeholder="Re-enter password" type="password" value={form.confirm} onChange={v=>f("confirm",v)} icon="lock"/>}
            </>}
        ...
        {tab==="account"&&<>
          ...
          <div style={{ marginTop:24 }}>
            ...
            <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", border:"1px solid #f0f0f0" }}>
              {[
                ["lock", "Privacy Policy",   "https://cashcounter.vbookng.com/privacy"],
                ["copy", "Terms of Service", "https://cashcounter.vbookng.com/terms"],
                ["mail", "Contact Support",  "mailto:v.bookenterprise@gmail.com"],
              ].map(([icon, label, href]) => (
      ...
      return (
        <div style={{ position:"fixed", inset:0, background:"#f7f9f8",
          ...
          <div style={{ fontSize:52, marginBottom:16 }}>
            {this.state.isIndexedDB ? "refresh" : ""}
          </div>
...
// Shown once to first-time visitors before the auth screen.
// Stored in localStorage so it never shows again after dismissed.
// ═══════════════════════════════════════════════════════════════
const WELCOME_SCREENS = [
  {
    ...
    headline: "Every naira in.\nEvery naira out.",
    body: "Record your income and expenses in seconds. Tag each entry to a category and let Cash Counter do the maths — automatically.",
  },
  {
    icon: (
      <svg viewBox="0 0 56 56" fill="none" style={{ width:56, height:56 }}>
  ...
  },
  {
    ...
    body: "Link your Nigerian bank account and Cash Counter imports your transactions automatically. No more manual entry — your records stay up to date.",
  },
];
...
export default function CashCounter() {
  ...
  // Show welcome screens to first-time visitors only
  const [showWelcome, setShowWelcome] = useState(
    () => !localStorage.getItem("cc_welcome_done")
  );
  ...
  if (showWelcome && !user) return (<ErrorBoundary><GlobalStyles/><WelcomeScreen onDone={handleWelcomeDone}/></ErrorBoundary>);
